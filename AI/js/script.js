...
//中間filter-mid UL
const filterMid = document.querySelector(".filter-mid");
//左邊filter-type
const filterListType = document.querySelector(".filter-list-type");
//filter-ai
const filterListAI = document.querySelector(".filter-list-ai");

function renderCategories() {
  let worksData2 = worksData;
  let categoriesOuter = '<li class="btn-category" id="btns">全部</li>';
  let categoriesInner = '<li class="filter-type">所有類型</li>';
  let filterAI = '<li class="filter-ai">所有模型</li>';
  ...
  works.forEach((item) => {
    categoriesOuter += `<li class="btn-category" id="btns">${item}</li>`;
    categoriesInner += `<li class="filter-type">${item}</li>`;
  });

  ai.forEach((item) => {
    filterAI += `<li class="filter-ai">${item}</li>`;
  });

  filterMid.innerHTML = categoriesOuter;
  filterListType.innerHTML = categoriesInner;
  filterListAI.innerHTML = filterAI;
}
